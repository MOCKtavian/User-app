...
namespace Framework\Routing;
    ...
    /**
     * Workaround for private invoke function
...
class BramusRouter
{
    ...
    public function getRequestHeaders()
    {
        ...
        foreach ($_SERVER as $name => $value) {
            if ((substr($name, 0, 5) == 'HTTP_') || ($name == 'CONTENT_TYPE') || ($name == 'CONTENT_LENGTH')) {
                $headers[str_replace(array(' ', 'Http'),
                    array('-', 'HTTP'),
    ...
    public function getRequestMethod()
    {
        ...
        elseif ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (isset($headers['X-HTTP-Method-Override']) && in_array(
            ...
            foreach ($this->notFoundCallback as $route_pattern => $route_callable) {
                ...
                if ($is_match) {
                    ...
                    $params = array_map(function ($match, $index) use ($matches) {
                        ...
                        if (isset($matches[$index + 1]) && isset($matches[$index + 1][0]) && is_array(
                                $matches[$index + 1][0]
                            )) {
